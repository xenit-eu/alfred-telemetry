...
public class AlfrescoNodeMeterBinder implements MeterBinder {
    ...
    public AlfrescoNodeMeterBinder(NodeDAO nodeDAOComponent, AclDAO aclDAOComponent,
    ...
    public void bindTo(MeterRegistry registry) {
        ...
        Gauge.builder(DAO_PREFIX + ".maxNodeId", nodeDAO, wrapDAOFunction(NodeDAO::getMaxNodeId))
                .description("Gives the maximum node id from the repo")
                .register(registry);
        Gauge.builder(DAO_PREFIX + ".maxTxnId", nodeDAO, wrapDAOFunction(NodeDAO::getMaxTxnId))
                .description("Gives the last transaction id from the repo")
                .register(registry);
        TimeGauge.builder(DAO_PREFIX + ".maxTxnCommitTime", nodeDAO, baseTimeUnit,
                wrapDAOFunction(NodeDAO::getMaxTxnCommitTime))
                .description("Gives the last transaction timestamp from the repo")
                .register(registry);
        Gauge.builder(ACL_PREFIX + ".maxChangeSetId", aclDAO, wrapAclFunction((dummy) -> {
            ...
            return aclDAO.getMaxChangeSetIdByCommitTime(maxCommitTime);
                ...
                .register(registry);
        TimeGauge.builder(ACL_PREFIX + ".maxChangeSetCommitTime", aclDAO, baseTimeUnit,
