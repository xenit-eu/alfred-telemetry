 ...
 * <p>
 * Utility that can be used to invoke lambdas in a safe way. Primarily designed to help support generically typed
 * callbacks where {@link ClassCastException class cast exceptions} need to be dealt with due to class erasure.
 *
 * @author Phillip Webb
 * @since 0.1.1
 */
public final class LambdaSafe {
     ...
     *
     * @param callbackType the callback type (a {@link FunctionalInterface functional interface})
    ...
    protected abstract static class LambdaSafeCallback<C, A, SELF extends LambdaSafeCallback<C, A, SELF>> {
        ...
        /**
         * Use a specific filter to determine when a callback should apply. If no explicit filter is set filter will be
         * attempted using the generic type on the callback type.
         *
    ...
    public static final class Callback<C, A> extends LambdaSafeCallback<C, A, Callback<C, A>> {
         ...
         * @return the result of the invocation (may be {@link InvocationResult#noResult} if the callback was not
         * invoked)
         */
    ...
    public static final class Callbacks<C, A> extends LambdaSafeCallback<C, A, Callbacks<C, A>> {
         ...
         * @param invoker the invoker used to invoke the callback
         * @param <R> the result type
         * @return the results of the invocation (may be an empty stream if no callbacks could be called)
         */
    ...
    /**
     * {@link Filter} that matches when the callback has a single generic and primary argument is an instance of it.
     */
    ...
    /**
     * The result of a callback which may be a value, {@code null} or absent entirely if the callback wasn't suitable.
     * Similar in design to {@link Optional} but allows for {@code null} as a valid value.
     *
     * @param <R> the result type
     */
    public static final class InvocationResult<R> {
        ...
        /**
         * Return the result of the invocation or {@code null} if the callback wasn't suitable.
         *
